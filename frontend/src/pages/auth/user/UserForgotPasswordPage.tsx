 ...
 * Account recovery via Email OTP using React Hook Form & Zod.
 */
const UserForgotPasswordPage: React.FC = () => {
  ...
  const onSubmit = (data: ForgotPasswordInput) => {
    ...
    setTimeout(() => {
      ...
      navigate('/auth/user/reset-password');
    }, 400);
  };

  const handleFillDemo = () => {
    setValue('email', 'user@example.demo', { shouldValidate: true });
    setError(null);
  };
      ...
      <Card className="shadow-md border-border/80">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <CardContent className="space-y-4">
            ...
            >
              Fill demo email (user@example.demo)
            </button>
          </CardContent>
